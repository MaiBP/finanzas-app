-- audit_transactions/audit_accounts (202608010001, 202608190001) fire on delete too, inserting
-- into audit_logs(household_id, ...). When leave_household() left the household's remaining
-- shared transactions/accounts for the final `delete from households` to cascade away, those
-- triggers fired mid-cascade — by then the household row was already gone within the same
-- transaction, so the insert violated audit_logs_household_id_fkey and the whole thing rolled
-- back, silently leaving the member (and the household) exactly as they were.
...
begin
  ...
  if remaining_count = 0 then
    -- Delete audited tables explicitly, while the household row still exists, so their audit
    -- triggers can still insert a valid household_id. Only then delete the household itself,
    -- which cascades away whatever's left (categories, transaction_splits, invites, ... — none
    -- of them audited).
