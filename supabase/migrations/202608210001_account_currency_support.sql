-- Both transaction-creating RPCs relied on transactions.currency's column default ('EUR') instead
-- of setting it explicitly, so every movement was recorded as EUR regardless of which currency its
-- account actually holds. Now that accounts can be created in a currency other than EUR, each
-- transaction must inherit its account's currency at the moment it's created — looked up here so no
-- application call site needs to pass or duplicate it.
...
-- Editing a movement can reassign it to a different account (existing feature) — if that account
-- holds a different currency, the row's own currency must follow it too, or it would keep showing
-- the old currency's symbol while actually living under an account denominated in another one.
...
-- Preference only — decides which currency the summary dashboards treat as "home" and aggregate
-- into. Accounts in any other currency stay out of those totals and show separately; nothing here
-- converts between currencies.
alter table public.households add column if not exists base_currency char(3) not null default 'EUR';
alter table public.profiles add column if not exists personal_base_currency char(3) not null default 'EUR';
