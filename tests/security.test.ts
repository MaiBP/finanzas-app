...
describe("Telegram webhook",()=>{it("validates the exact secret",()=>{expect(isValidWebhookSecret("secret","secret")).toBe(true);expect(isValidWebhookSecret("wrong","secret")).toBe(false);expect(isValidWebhookSecret(null,"secret")).toBe(false)})});
