...
import { escapeTelegramHtml, withTelegramWebSuggestion } from "@/lib/telegram/api";
...
describe("Telegram webhook",()=>{it("validates the exact secret",()=>{expect(isValidWebhookSecret("secret","secret")).toBe(true);expect(isValidWebhookSecret("wrong","secret")).toBe(false);expect(isValidWebhookSecret(null,"secret")).toBe(false)});it("escapes dynamic HTML",()=>expect(escapeTelegramHtml('<Cuenta & "Casa">')).toBe("&lt;Cuenta &amp; &quot;Casa&quot;&gt;"));it("escapes replies before adding the web link",()=>{const message=withTelegramWebSuggestion("Gasto <personal>");expect(message).toContain("Gasto &lt;personal&gt;");expect(message).toContain('<a href="https://')})});
