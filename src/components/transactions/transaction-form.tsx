...
import { createTransaction, type ActionState } from "@/app/(dashboard)/app/actions";
import type { Account, Category } from "@/types/database";

const initialState: ActionState = {};
export function TransactionForm({ accounts, categories }: { accounts: Pick<Account,"id"|"name">[]; categories: Category[] }) {
  ...
  return <form action={action} className="space-y-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><label><span className="label">¿Para quién?</span><select className="field" name="scope" defaultValue="shared"><option value="shared">Compartido</option><option value="personal">Personal</option></select></label><label><span className="label">Privacidad</span><select className="field" name="privacy" defaultValue="visible"><option value="visible">Detalle visible</option><option value="private">Detalle privado</option></select></label></div>
    ...
    <button disabled={pending} className="w-full rounded-xl bg-[#26725c] px-5 py-3.5 font-bold text-white disabled:opacity-60">{pending ? "Guardando…" : "Guardar movimiento"}</button>
    <p className="text-center text-xs text-[#6c7f7a]">El reparto compartido se crea automáticamente entre los miembros activos.</p>
  </form>;
}
