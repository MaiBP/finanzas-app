...
import { answerCallbackQuery, downloadTelegramFile, editMessageReplyMarkup, MAX_TELEGRAM_IMPORT_BYTES, sendTelegramMessage, withTelegramWebSuggestion, type InlineKeyboardMarkup } from "@/lib/telegram/api";
...
async function handlePendingAccountSelection(db:ReturnType<typeof createAdminClient>,userId:string,householdId:string,text:string){
  ...
  if(action.requires_confirmation){const {error}=await db.from("pending_actions").update({payload:action}).eq("id",data.id);if(error)throw error;return {text:`Usaré ${selected.name}. Responde “sí” para confirmar el movimiento o “no” para cancelar.`,keyboard:confirmCancelKeyboard("create_transaction")};}
  const reply=await executeTelegramAction(db,userId,householdId,action); await db.from("pending_actions").delete().eq("id",data.id); return {text:reply};
}
...
async function handleCallbackQuery(callbackQuery:z.infer<typeof callbackQuerySchema>){
  ...
  try{
    if(data.startsWith("confirm:")){
      ...
      if(pending?.action_type!==actionType)reply="Esa opción ya no está disponible.";
      else if(choice==="yes")reply=await confirmPending(db,link.user_id,membership.household_id);
      ...
      reply=selection?.text??"Esa opción ya no está disponible."; keyboard=selection?.keyboard;
    }
    else return;
  }catch(error){reply=error instanceof Error?error.message:"No he podido completar eso. Inténtalo de nuevo.";}
  ...
  await sendTelegramMessage(chatId,withTelegramWebSuggestion(reply),keyboard).catch(()=>undefined);
}
...
export async function POST(request:Request){
  ...
  try{
    ...
    const [{data:categories},{data:accounts},recent]=await Promise.all([db.from("categories").select("name,kind").or(`household_id.eq.${membership.household_id},household_id.is.null`),db.from("accounts").select("name,is_shared").eq("household_id",membership.household_id).neq("type","joint").is("archived_at",null).or(`owner_user_id.eq.${link.user_id},is_shared.eq.true`),fetchRecentMessages(db,link.user_id)]);
    ...
    else {await queueAction(db,link.user_id,membership.household_id,action);reply=action.action==="delete_transaction"?"He encontrado la acción de borrado. Responde “sí” para confirmarla o “no” para cancelar.":"Queda pendiente. Responde “sí” para confirmar o “no” para cancelar.";keyboard=confirmCancelKeyboard(action.action);}
