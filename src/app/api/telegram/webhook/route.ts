...
async function confirmPending(db:ReturnType<typeof createAdminClient>,userId:string,householdId:string){
  ...
  const action=financialActionSchema.parse(data.payload); let reply:string;
  if(action.action==="create_transaction") reply=await executeTelegramAction(db,userId,householdId,action);
