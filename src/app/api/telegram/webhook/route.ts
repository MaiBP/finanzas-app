...
import { getMonthSummary, getRecentTransactions, getRecordedBalance } from "@/services/query-service";
...
export async function POST(request:Request){
  ...
  try{
    ...
    else if(action.action==="query_finances"){const scope=action.data.filters.scope??"combined";reply=action.data.query_type==="recent_transactions"?await getRecentTransactions(db,membership.household_id,link.user_id,5,scope):action.data.query_type==="household_balance"?await getRecordedBalance(db,membership.household_id,link.user_id,scope):await getMonthSummary(db,membership.household_id,link.user_id,new Date(),scope);}
