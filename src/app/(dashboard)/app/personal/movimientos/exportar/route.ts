...
export async function GET(request: Request) {
  ...
  const currency = requestedCurrency && availableCurrencies.has(requestedCurrency) ? requestedCurrency : baseCurrency;
  ...
  const header = ["Fecha", "Tipo", "Descripción", "Categoría", "Cuenta", `Importe (${currency})`, "Producto", "Subcategoría producto", `Importe producto (${currency})`];
  ...
  const currencySuffix = currency !== baseCurrency ? `-${currency.toLowerCase()}` : "";
  return new Response(`﻿${lines.join("\r\n")}`, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="miti-miti-movimientos-personales${currencySuffix}-${dateSuffix}.csv"`,
