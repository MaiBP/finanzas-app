...
export async function askAssistant(_state:AssistantState,formData:FormData):Promise<AssistantState>{
  ...
  try{const [{data:categories},{data:accounts}]=await Promise.all([supabase.from("categories").select("name,kind").or(`household_id.eq.${household.id},household_id.is.null`),supabase.from("accounts").select("name,is_shared").eq("household_id",household.id).is("archived_at",null)]);const action=await parseFinancialMessage({text,userId:user.id,householdId:household.id,now:new Date().toISOString(),categories:categories??[],accounts:accounts??[],recentMessages:[]});
    if(action.action==="query_finances")return{reply:action.data.query_type==="recent_transactions"?await getRecentTransactions(supabase,household.id,user.id):await getMonthSummary(supabase,household.id,user.id)};
