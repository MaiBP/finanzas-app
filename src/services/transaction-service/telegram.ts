...
export async function executeTelegramAction(db: SupabaseClient, userId:string, householdId:string, action:FinancialAction) {
  ...
  let accountQuery=db.from("accounts").select("id,name").eq("household_id",householdId).is("archived_at",null).or(`owner_user_id.eq.${userId},is_shared.eq.true`);
  ...
  if(!category||!account) throw new Error("No encuentro una categoría o cuenta válida; registra el movimiento desde la web.");
