...
export const financialActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("query_finances"), confidence: z.number().min(0).max(1), requires_confirmation: z.literal(false), data: z.object({ query_type: z.enum(["month_summary","category_spending","user_contributions","household_balance","recent_transactions","compare_months","spending_by_date_range"]), filters: z.object({category:z.string().nullable(),user_name:z.string().nullable(),date_from:z.iso.date().nullable(),date_to:z.iso.date().nullable(),month:z.string().nullable(),scope:z.enum(["shared","personal","combined"]).nullable()}) }) }),
